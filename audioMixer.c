...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	if (!foundSlot) {
		printf("ERROR: No free slot found. Unable to queue sound bite.\n");
	}
...
{
	memset(playbackBuffer, 0, sizeof(*playbackBuffer) * playbackBufferSize);

	pthread_mutex_lock(&audioMutex);
	{
		for (int i = 0; i < MAX_SOUND_BITES; i++) {
			if (soundBites[i].pSound != NULL) {
				int currLocation = soundBites[i].location;
				int currNumSamples = soundBites[i].pSound->numSamples;
				short *currPData = soundBites[i].pSound->pData;
				int maxSize = size;
				if ((currLocation + size) >= currNumSamples) {
					maxSize = currNumSamples - currLocation;
				}
				// Add sound bite data to playback buffer
				for (int j = 0; j < maxSize; j++) {
					long pcmSum = playbackBuffer[j] + currPData[currLocation];
					...
					playbackBuffer[j] = pcmSum;
					currLocation++;
				}
				// Free slot if the entire sample has been played back
				if (currLocation == currNumSamples) {
					soundBites[i].pSound = NULL;
				}
				soundBites[i].location = currLocation;
			}
		}
	}
	pthread_mutex_unlock(&audioMutex);
